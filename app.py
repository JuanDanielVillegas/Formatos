...
def find_emp(nombre): 
    ...
    try:
        ...
        depto_value = fix_y(rows[0].Departamento.title()) if rows[0].Departamento is not None else alert
        title_value = rows[0].titulo.upper() if rows[0].titulo is not None else 'C.'
        email_value = rows[0].correo.lower().replace('buzon.', '') if rows[0].correo is not None else alert
